...
export class InjectionException extends RuntimeException {
    ...
    constructor(definition: ServiceDefinition, injection: Injection, previous: Error) {
        super(InjectionException.getMessage(definition, injection), previous);
        this.definition = definition;
        this.injection = injection;
    }

    private static getMessage(definition: ServiceDefinition, injection: Injection): string {
        let message = `Injection for identifier "${injection.serviceIdentifier}" failed. Requested by service "${definition.getName()}" `;
